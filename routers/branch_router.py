...
def edit_application_page(request: Request, app_id: int, db: Session = Depends(get_db)):
    user, redir = _check(request)
    if redir:
        return redir
    ...
    return templates.TemplateResponse(
        "branch/edit_application.html",
        {"request": request, "session": request.session, "app": app},
    )


@router.post("/applications/{app_id}/edit")
async def update_application(request: Request, app_id: int, db: Session = Depends(get_db)):
    user, redir = _check(request)
    if redir:
        return redir
    ...
    app.title = form.get("title", app.title)
